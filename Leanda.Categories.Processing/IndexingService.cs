...
namespace Leanda.Categories.Processing
{
    public class IndexingService : IMicroService
    {
        ...
        public void Start()
        {
            ...
            try
            {
                var a = new { Index = "categories", Type = "category" };
                CreateMappings(elasticClient, a.Index, a.Type).Wait();
                SetupAnlyzer(elasticClient, a.Index).Wait();
            ...
            {
                Sds.Heartbeat.TcpPortListener.Start(port);
            }
        }
